...
export const updateBlog = async (req, res) => {
  try {
    const blog_to_update = await Listing.findById(req.params.id);
    if (blog_to_update.user_id === req.body.userId) {
      await blog_to_update.updateOne({ $set: req.body });
      res.status(200).json("the blog_to_update has been updated");
    } else {
      res.status(403).json("you can update only your blog_to_update");
    }
  } catch (err) {
    res.status(500).json(err);
  }
...
export const deleteBlog = async (req, res) => {
  try {
    const blog_to_delete = await Listing.findById(req.params.id);
